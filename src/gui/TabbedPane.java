...
public class TabbedPane
{
   ...
   private Node uebersicht_Contet()
   {
      ...
      listUebersicht.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
         ...
         konsoloe.getSelectionModel().select(tab);
      ...
      ueberschrift.setFont(Util.ueberschriftFont);
      border.setTop(ueberschrift);

      Label kurzeInfoUeberFelder = new Label(
            I18n.getText("Number_of_fields_from") + " " + Nutzer.aktuellerNutzer.getName() + ": " + anzFelderDesNutzers);
   ...
   private Node rechte_Content()
   {
      ...
      //todo warum "0" und nicht index 0?
      speicherart.getSelectionModel().select(Nutzer.aktuellerNutzer.getSpeichart());
      speicherart.addEventHandler(ActionEvent.ACTION, (e) -> speichernCombobox(speicherart.getSelectionModel().getSelectedIndex()));
      ...
      comboBoxSprachen.addEventHandler(ActionEvent.ACTION, (e) -> sprachenCombobox(comboBoxSprachen.getSelectionModel().getSelectedIndex()));
   ...
   private void speichernCombobox(Integer selected)
   {
      Nutzer.aktuellerNutzer.setSpeichart(selected);
   }

   private void sprachenCombobox(Integer selected)
   {
   ...
   public static Integer getVorrausichlicheErnte(Feld feld)
   {
      ...
      {
         if( pflanze.getHoehe() + pflanze.getPflanzenArten().getWachstum() * 1.5 >= pflanze.getPflanzenArten().getErntehoehe() )
         {
